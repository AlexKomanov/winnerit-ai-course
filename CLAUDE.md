...
- **`.step-action`** (light cream block inside a step-block): for UI navigation steps — clicking, searching, opening menus, typing into a chat box. Uses `var(--bg-2)` background with plain text. **Never put `<span class="arr">` inside `.step-action`** — those orange arrow spans were removed from all files and must not be re-introduced. Never use `.step-code` for UI actions; it looks like a terminal command and confuses students.
- **`.step-action` with all-English navigation sequences:** wrap content in `<span dir="ltr">…</span>` and use `→` as the separator between steps. This prevents the browser's bidi algorithm from reversing the visual order. Example: `<span dir="ltr"><strong>/plugin</strong> → Discover → <strong>Install for you</strong></span>`. Use `←` separators only when the sequence contains Hebrew text that anchors RTL base direction.
...
- **Custom grid for step-blocks:** when `.steps-grid` (3-col) or `.steps-grid.two` (2-col) don't fit, use an inline grid: `<div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-top:32px;">`. Adjust column count as needed. Step-blocks go inside as normal.
- **Full-width step-block in a custom grid:** add `style="grid-column:1/-1;"` to span all columns. Use for intro/summary cards at the top of a multi-column layout.
- **Bullet lists inside step-blocks:** for capability or settings lists (not a single paragraph), use `<ul style="list-style:none;padding:0;margin:10px 0 0;display:flex;flex-direction:column;gap:6px;font-size:15px;color:var(--ink-soft);line-height:1.4;">` with `<li>· text</li>` items. Minimum 15px font size.
- **Inline code inside step-block paragraphs:** use a plain `<code>` element — no inline style needed. The `.step-block code` CSS rule styles it automatically: coral color, coral-tinted background, border, rounded corners, monospace font, `margin: 0 3px` spacing. Use for CLI flags (`--transport http`), separators (`--`), env var names (`API_KEY`), and other command-related terms that appear inside Hebrew paragraph text. CSS rule to include in every presentation:
  ```css
  .step-block code { font-family: var(--mono); font-size: 12px; direction: ltr; unicode-bidi: embed; background: rgba(224,78,54,0.08); border: 1px solid rgba(224,78,54,0.25); border-radius: 4px; padding: 2px 7px; margin: 0 3px; color: var(--coral); vertical-align: middle; display: inline-block; line-height: 1.5; }
  ```
